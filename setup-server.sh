...
sudo mkdir -p /var/www/webapp/marabuweb/{current,backups,temp,logs}
sudo chown -R $USER:$USER /var/www/webapp/marabuweb

# Rendre le script de déploiement exécutable
echo "🔧 Configuration des permissions..."
chmod +x /var/www/webapp/marabuweb/deploy.sh
...
sudo tee /etc/nginx/sites-available/marabu.services << EOF
server {
    listen 80;
...
# Activer le site
sudo ln -s /etc/nginx/sites-available/marabu.services /etc/nginx/sites-enabled/
sudo nginx -t
